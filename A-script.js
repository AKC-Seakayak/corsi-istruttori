...
document.addEventListener('DOMContentLoaded', () => {
        ...
        for (let i = 0; i < courseStudents.length; i++) {
            ...
            studentsHtml += `
                <div style="${pageBreak}">
                    ...
                    <table>
                        ...
                        <tbody>
                            ${evaluationItems.map(item => {
                                ...
                                return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note || '')}</table>\d`;
                            }).join('')}
                        </tbody>
                    </table>
                    ...
                    <table><th>Note</th><tr>${escapeHtml(s.psicoAttitudinali?.note || '')}</td></tr>
                    <tr>
    ...
    async function printSingleStudent(student, title = "Stampa Scheda") {
        ...
        const studentHtml = `
            ...
            <h4>Valutazioni didattiche</h4>
            </tr><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead>
            <tbody>
                ${didatticheItems.map(item => {
            ...
            <h4>Capacità psico-attitudinali</h4>
            <table><th>Valutazione</th><td>${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr>
    ...
    // Listener per il pulsante di stampa singola (ID: print-student-btn)
    ...
    async function showStudentEditView(student) {
        currentEditingStudent = student;   // IMPORTANTE per la stampa singola
        document.getElementById('courses-view').style.display = 'none';
