...
let coursesList = [];
let allUsersList = [];
let currentEditingStudent = null;   // Per la stampa singola
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const printPdfCourseBtn = document.getElementById('print-pdf-course-btn');
    ...
    // Pulsanti stampa corso
    printCourseBtn.addEventListener('click', () => printCourse("Stampa Corso"));
    if (printPdfCourseBtn) {
        printPdfCourseBtn.addEventListener('click', () => printCourse("PDF Corso"));
    }
    ...
    async function printSingleStudent(student, title = "Stampa Scheda") {
        ...
        const studentHtml = `
            ...
            <table>
                ...
                <tbody>
                    ${evaluationItems.map(item => {
                        ...
                        return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note || '')}</td></td>`;
                    }).join('')}
                </tbody>
            </table>
            ...
            <h4>Capacità psico-attitudinali</h4>
            <td><th>Valutazione</th><td>${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr>
    ...
    // Pulsanti per la scheda singola
    const printStudentPdfBtn = document.getElementById('print-student-pdf-btn');
    const saveStudentPdfBtn = document.getElementById('save-student-pdf-btn');
    if (printStudentPdfBtn) {
        printStudentPdfBtn.addEventListener('click', () => {
    ...
    if (saveStudentPdfBtn) {
        saveStudentPdfBtn.addEventListener('click', () => {
            if (currentEditingStudent) {
                printSingleStudent(currentEditingStudent, "PDF Scheda (Salva)");
            } else {
                alert('Nessuna scheda aperta');
            }
        });
    ...
    // ===================== FUNZIONI ESISTENTI (modificate per supporto stampa singola) =====================
    ...
    async function showStudentEditView(student) {
        currentEditingStudent = student;   // Salva lo studente per la stampa singola
        document.getElementById('courses-view').style.display = 'none';
