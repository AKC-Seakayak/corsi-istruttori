...
async function printCourse(title = "Stampa corso") {
    ...
    for (let i = 0; i < courseStudents.length; i++) {
        ...
        studentsHtml += `<div style="${pageBreak}">
            ...
            <h4>Attrezzatura</h4>
            <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(s.attrezzatura?.score)}</td></tr><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(s.attrezzatura?.note || '')}</td></tr></table>
            ...
            <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(s.psicoAttitudinali?.score)}</td></tr><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(s.psicoAttitudinali?.note || '')}</td></tr></table>
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `${logoHtml}
        ...
        <table style="width:100%; border-collapse:collapse;">${techRows}</table>
        <h4>Attrezzatura</h4>
        <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(student.attrezzatura?.score)}</td></tr><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(student.attrezzatura?.note || '')}</td></tr></table>
        <h4>Valutazioni didattiche</h4>
        <table style="width:100%; border-collapse:collapse;"><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${didatRows}</tbody></table>
        <h4>Capacità psico-attitudinali</h4>
        <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(student.psicoAttitudinali?.note || '')}</td></tr></table>
        <h4>Giudizio finale e TESI esame</h4>
...
// ===================== ESPORTAZIONE PDF (con fallback a stampa su iOS) =====================
async function exportCourseAsPDF() {
    if (isIOS()) {
        alert("Su iPhone/iPad l'esportazione PDF viene gestita tramite la stampa. Premi 'Stampa' e poi scegli 'Salva come PDF'.");
        await printCourse("Stampa corso (salva come PDF)");
        return;
    }
    if (!currentCourseId) return;
    const course = coursesList.find(c => c.id === currentCourseId);
    ...
    for (let i = 0; i < courseStudents.length; i++) {
        ...
        for (let item of evaluationItems) {
            const v = s.evaluations?.[item] || {};
            techHtml += `<tr><td style="border:1px solid #ccc; padding:4px;"><strong>${escapeHtml(item)}</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(v.score)}</td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(v.note || '')}</td></tr>`;
        }
        let didatHtml = '';
        for (let item of didatticheItems) {
            const v = s.didattiche?.[item] || {};
            didatHtml += `<tr><td style="border:1px solid #ccc; padding:4px;"><strong>${escapeHtml(item)}</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(v.score)}</td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(v.note || '')}</td></tr>`;
        }
        studentsHtml += `<div style="${pageBreak}">
            ${logoHtml}
            <h3>${escapeHtml(s.name)} ${escapeHtml(s.surname)}</h3>
            ...
            <strong>Altri brevetti:</strong> ${escapeHtml(s.otherPatents || '-')}</p>
            <h4>Valutazioni tecniche</h4>
            <table style="width:100%; border-collapse:collapse;">${techHtml}</table>
            <h4>Attrezzatura</h4>
            <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(s.attrezzatura?.score)}</td></tr><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(s.attrezzatura?.note || '')}</td></tr></table>
            <h4>Valutazioni didattiche</h4>
            <table style="width:100%; border-collapse:collapse;"><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${didatHtml}</tbody></table>
            <h4>Capacità psico-attitudinali</h4>
            <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(s.psicoAttitudinali?.score)}</td></tr><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(s.psicoAttitudinali?.note || '')}</td></tr></table>
            <h4>Giudizio finale e TESI esame</h4>
            <p>${escapeHtml(s.giudizioFinale || '')}</p>
    ...
    const opt = {
        ...
        filename: `${course.name}_completo.pdf`,
    ...
    try {
        ...
        showMessage('Errore PDF: ' + err.message, 'error');
    } finally {
...
async function exportStudentAsPDF() {
    if (isIOS()) {
        alert("Su iPhone/iPad l'esportazione PDF viene gestita tramite la stampa. Premi 'Stampa' e poi scegli 'Salva come PDF'.");
        await printSingleStudent(currentEditingStudent, "Stampa scheda (salva come PDF)");
        return;
    }
    if (!currentEditingStudent) return alert("Nessun allievo selezionato");
    ...
    const opt = {
        margin: [0.5, 0.5, 0.5, 0.5],
        filename: `${student.name}_${student.surname}_scheda.pdf`,
        image: { type: 'jpeg', quality: 0.95 },
        html2canvas: { scale: 1, logging: false, useCORS: true },
        jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' }
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function showStudentEditView(student) {
        ...
        for (let item of evaluationItems) {
            ...
            techRows += `<tr><td style="font-weight:bold;">${safeItem}</td><td><select class="evaluation-select" data-type="tech" data-item="${safeItem}">${makeSelect(val)}</select></td><td><div class="note-cell"><textarea class="note-textarea" data-type="tech" data-item="${safeItem}" rows="2">${escapeHtml(note)}</textarea><button class="delete-note-btn" data-type="tech" data-item="${safeItem}">🗑 Cancella</button></div></td></tr>`;
        }
        let didatRows = '';
        for (let item of didatticheItems) {
            ...
            const safeItem = escapeHtml(item);
            didatRows += `<tr><td style="font-weight:bold;">${safeItem}</td><td><select class="evaluation-select" data-type="didat" data-item="${safeItem}">${makeSelect(val)}</select></td><td><div class="note-cell"><textarea class="note-textarea" data-type="didat" data-item="${safeItem}" rows="2">${escapeHtml(note)}</textarea><button class="delete-note-btn" data-type="didat" data-item="${safeItem}">🗑 Cancella</button></div></td></td>`;
        }
