...
async function exportCourse(format) {
    ...
    const data = allievi.map(s => ({
        "Nome": s.name, "Cognome": s.surname, "Data di nascita": s.birthDate, "Indirizzo": s.address,
        "Telefono": s.phone, "Email": s.email, "Cert. medico": s.medicalCert ? "Sì" : "No",
...
async function exportStudent(format) {
    ...
    const data = [{
        "Corso": course?.name || "", "Nome": s.name, "Cognome": s.surname, "Data di nascita": s.birthDate,
...
async function printCourse(opt_title = "Stampa corso") {
        ...
        studentsHtml += `
            <div style="${pageBreak}">
                ...
                <table border="1" cellpadding="4">
                    ...
                    <tr><th>Note</th><td>${escapeHtml(s.psicoAttitudinali?.note || '')}</td></td>
                </table>
    ...
    const fullHtml = `<!DOCTYPE html><html><head><title>${escapeHtml(course.name)}</title><style>body{font-family:Inter,sans-serif;margin:20px} table{border-collapse:collapse;width:100%} th,td{border:1px solid #ccc;padding:4px} img{max-width:180px}</style></head><body><h2>${escapeHtml(course.name)}</h2><p>Stampato il ${new Date().toLocaleString()} | Livello: ${course.level === 1 ? 'Base' : 'Avanzato'}</p>${studentsHtml}</body></html>`;
    const printWindow = window.open('', '_blank');
    if (!printWindow) { alert("Permetti i popup per questo sito per stampare."); return; }
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `
        ...
        <h4>Valutazioni tecniche</h4>
        <table border="1" cellpadding="4">
            <thead><tr><th>Abilità</th><th>Valutazione</th><th>Note</th></td></thead>
            <tbody>
                ${evaluationItems.map(item => {
        ...
        <table border="1" cellpadding="4">
            ...
            <tr><th>Note</th><td>${escapeHtml(student.attrezzatura?.note || '')}NonNull} -->
        </tr>
        <h4>Valutazioni didattiche</h4>
        <table border="1" cellpadding="4">
            <thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead>
            <tbody>
                ${didatticheItems.map(item => {
                    ...
                    return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}<td><td>${escapeHtml(v.note || '')}NonNull} -->
                `}).join('')}
            </tbody>
        </table>
        <h4>Capacità psico-attitudinali</h4>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const exportCoursePdfBtn = document.getElementById('export-course-pdf-btn');
    if (exportCoursePdfBtn) exportCoursePdfBtn.addEventListener('click', exportCourseAsPDF);
    const exportStudentPdfBtn = document.getElementById('export-student-pdf-btn');
    if (exportStudentPdfBtn) exportStudentPdfBtn.addEventListener('click', exportStudentAsPDF);
        ...
        try {
            ...
            for (const row of rows) {
                const studentData = {
                    ...
                    phone: row["Telefono"] || "",
                    email: row["Email"] || "",
                    medicalCert: row["Cert. medico"] === "Sì",
                    blsdCert: row["Cert. BLSD"] === "Sì",
                    otherPatents: row["Altri brevetti"] || "",
                    giudizioFinale: row["Giudizio finale e TESI esame"] || "",
                    evaluations: {},
                    didattiche: {},
                ...
                for (let item of evaluationItems) {
                    studentData.evaluations[item] = { score: row[item] || "0", note: "" };
                }
                for (let item of didatticheItems) {
                    studentData.didattiche[item] = { score: row[item] || "0", note: "" };
                }
            ...
            showMessage("Errore durante l'importazione: " + err.message, "error");
        }
    ...
    // ===================== SCHEDA ALLIEVO CON ACCORDION SEPARATI =====================
    async function showStudentEditView(student) {
        ...
        const formHTML = `
            <div class="form-section">
                ...
                <!-- ACCORDION 5: GIUDIZIO FINALE E TESI ESAME -->
                <div class="accordion-section" id="accordion-giudizio">
                    <div class="accordion-header">Giudizio finale e TESI esame</div>
                    <div class="accordion-content">
                ...
                <!-- PULSANTI SALVA E ANNULLA (FUORI DA OGNI ACCORDION) -->
        ...
        // Cancellazione note tecniche e didattiche
