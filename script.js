...
async function importStudentsFromExcel(file, courseId) {
...
// ===================== FUNZIONI DI SALVATAGGIO PDF (html2pdf) =====================
async function saveCourseAsPDF() {
    ...
    const fullHtml = `
        ...
        <html>
        <head>
            <title>${escapeHtml(course.name)} - PDF Corso</title>
            <style>
        ...
        <body>
            <h2>${escapeHtml(course.name)} - Riepilogo completo</h2>
            <p>Data di esportazione: ${new Date().toLocaleString()} | Livello: ${course.level === 1 ? 'Base' : 'Avanzato'}</p>
            ${studentsHtml}
        </body>
        </html>
    `;
    ...
    try {
        ...
        showMessage('PDF generato e salvato!', 'success');
    } catch (err) {
        showMessage('Errore durante la generazione del PDF: ' + err.message, 'error');
    }
}

async function saveStudentAsPDF() {
    if (!currentEditingStudent) return alert("Nessun allievo selezionato");
    ...
    const studentHtml = `
        ...
        <h4>Valutazioni didattiche</h4>
        <table border="1" cellpadding="4" style="width:100%; margin-bottom:15px;">
            <thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></table></thead>
            <tbody>
                ${didatticheItems.map(item => {
                    const v = student.didattiche?.[item] || {};
                    return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note || '')}</td>｜｜DSML｜｜`;
                }).join('')}
            </tbody>
        <table>
        <h4>Capacità psico-attitudinali</h4>
    ...
    const fullHtml = `
        ...
        <body>
            ...
            <p>Data di esportazione: ${new Date().toLocaleString()}</p>
            ${studentHtml}
        </body>
        </html>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function printSingleStudent(student, title = "Stampa Scheda") {
        ...
        const studentHtml = `
            ...
            <table border="1" cellpadding="4" style="width:100%; margin-bottom:15px;">
                <tr><th>Valutazione</th><td>${getValutazioneLabel(student.attrezzatura?.score)}</td></tr>
                <td><th>Note</th><td>${escapeHtml(student.attrezzatura?.note || '')}NonNull} -->
            </table>
            ...
            <table border="1" cellpadding="4" style="width:100%; margin-bottom:15px;">
                ...
                <tbody>
                    ${didatticheItems.map(item => {
                        const v = student.didattiche?.[item] || {};
                        return `<tr><td><strong>${escapeHtml(item)}</strong></td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note || '')}NonNull} -->
                    `}).join('')}
                </tbody>
            </table>
            <h4>Capacità psico-attitudinali</h4>
    ...
    // ===================== LISTENER PER I NUOVI PULSANTI SALVATAGGIO PDF =====================
    const saveCourseBtn = document.getElementById('save-course-pdf-btn');
    if (saveCourseBtn) {
        saveCourseBtn.addEventListener('click', saveCourseAsPDF);
    }
    const saveStudentBtn = document.getElementById('save-student-pdf-btn');
    if (saveStudentBtn) {
        saveStudentBtn.addEventListener('click', saveStudentAsPDF);
    }
    ...
    // ===================== FUNZIONI PRINCIPALI (CRUD corsi/allievi) =====================
