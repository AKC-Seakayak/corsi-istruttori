...
// Stampa con iframe fuori schermo (nessuna finestra popup persistente)
async function printCourse(title = "Stampa corso") {
    ...
    for (let i = 0; i < courseStudents.length; i++) {
        ...
        studentsHtml += `<div style="${pageBreak}">${logoHtml}
            ...
            <h4>Attrezzatura</h4><table><td><th>Valutazione</th><td>${getValutazioneLabel(s.attrezzatura?.score)}</td></tr>
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `${logoHtml}
        ...
        <h4>Attrezzatura</h4><table border="1"><tr><th>Valutazione</th><td>${getValutazioneLabel(student.attrezzatura?.score)}</td></tr>
        ...
        <h4>Capacità psico-attitudinali</h4><table border="1"><tr><th>Valutazione</th><td>${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr>
