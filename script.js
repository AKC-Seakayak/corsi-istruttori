...
async function exportCourse(format) {
    ...
    const data = allievi.map(s => ({
        "Nome": s.name,
        "Cognome": s.surname,
        "Data di nascita": s.birthDate,
        "Indirizzo": s.address,
        "Telefono": s.phone,
        "Email": s.email,
        "Cert. medico": s.medicalCert ? "Sì" : "No",
        "Cert. BLSD": s.blsdCert ? "Sì" : "No",
        "Altri brevetti": s.otherPatents,
        "Giudizio finale e TESI esame": s.giudizioFinale,
        ...(s.evaluations || {}),
        ...(s.didattiche || {}),
        "Attrezzatura_valutazione": s.attrezzatura?.score,
        "Attrezzatura_note": s.attrezzatura?.note,
        "Psico_valutazione": s.psicoAttitudinali?.score,
        "Psico_note": s.psicoAttitudinali?.note,
    }));
...
// ===================== STAMPA CON NUOVA FINESTRA (funziona su tutti i browser) =====================
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `
        ...
        <h4>Valutazioni tecniche</h4>
        <table border="1" cellpadding="4">
            <thead></table><th>Abilità</th><th>Valutazione</th><th>Note</th></tr></thead>
            <tbody>
                ${evaluationItems.map(item => {
...
// ===================== ESPORTAZIONE PDF (con html2pdf, funziona su tutti) =====================
        ...
        const formHTML = `
            <div class="form-section">
                ...
                <div class="accordion-section" id="accordion-didat">
                    ...
                    <div class="accordion-content"><div style="overflow-x:auto;"><table class="evaluation-table"><tbody>${didatRows}</tbody><tr></div></div>
                </div>
