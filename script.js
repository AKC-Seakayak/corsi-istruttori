...
async function printCourse(title = "Stampa corso") {
    ...
    for (let i = 0; i < courseStudents.length; i++) {
        ...
        for (let item of evaluationItems) {
            ...
            techRows += `<tr><td><strong>${escapeHtml(item)}</strong></td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note || '')}</td></td>`;
        }
        let didatRows = '';
        ...
        studentsHtml += `<div style="${pageBreak}">
            ...
            <h4>Valutazioni tecniche</h4><table border="1" cellpadding="4" style="border-collapse:collapse; width:100%;"><thead><tr><th>Abilità</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${techRows}</tbody></table>
            <h4>Attrezzatura</h4><table border="1" cellpadding="4"><tr><th>Valutazione</th><td>${getValutazioneLabel(s.attrezzatura?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(s.attrezzatura?.note || '')}</td></tr></table>
            <h4>Valutazioni didattiche</h4><table border="1" cellpadding="4" style="width:100%;"><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${didatRows}</tbody></table>
            <h4>Capacità psico-attitudinali</h4><table border="1" cellpadding="4"><tr><th>Valutazione</th><td>${getValutazioneLabel(s.psicoAttitudinali?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(s.psicoAttitudinali?.note || '')}</td></tr></table>
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `${logoHtml}
        ...
        <h4>Valutazioni tecniche</h4><table border="1" cellpadding="4" style="border-collapse:collapse; width:100%;"><thead><tr><th>Abilità</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${techRows}</tbody></table>
        <h4>Attrezzatura</h4><table border="1" cellpadding="4"><tr><th>Valutazione</th><td>${getValutazioneLabel(student.attrezzatura?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(student.attrezzatura?.note || '')}</td></tr></table>
        <h4>Valutazioni didattiche</h4><table border="1" cellpadding="4" style="width:100%;"><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${didatRows}</tbody></table>
        <h4>Capacità psico-attitudinali</h4><table border="1" cellpadding="4"><tr><th>Valutazione</th><td>${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(student.psicoAttitudinali?.note || '')}</td></tr></table>
...
// ===================== ESPORTAZIONE PDF (OTTIMIZZATA PER MOBILE) =====================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function showStudentEditView(student) {
        ...
        for (let item of didatticheItems) {
            ...
            const safeItem = escapeHtml(item);
            didatRows += `</td><td style="font-weight:bold;">${safeItem}</td><td><select class="evaluation-select" data-type="didat" data-item="${safeItem}">${makeSelect(val)}</select></td><td><div class="note-cell"><textarea class="note-textarea" data-type="didat" data-item="${safeItem}" rows="2">${escapeHtml(note)}</textarea><button class="delete-note-btn" data-type="didat" data-item="${safeItem}">🗑 Cancella</button></div></td></tr>`;
        }
