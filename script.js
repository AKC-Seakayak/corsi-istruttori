...
// ===================== FUNZIONI DI STAMPA (IFRAME PER iOS/Mac, WINDOW.OPEN PER ANDROID) =====================
async function printCourse(opt_title = "Stampa Corso") {
    ...
    if (isAndroid()) {
        // Android: apre una nuova finestra (popup) e inietta il contenuto
        const printWindow = window.open('', '_blank');
        if (!printWindow) {
            alert("Il browser ha bloccato il popup. Consenti i popup per questo sito per stampare.");
            return;
        }
        ...
        printWindow.focus();
        printWindow.print();
        // Non chiudiamo automaticamente per dare il tempo di stampare – l'utente chiuderà la scheda.
    } else {
        // iOS / macOS / altri: usa iframe nascosto (funziona perfettamente)
...
async function printSingleStudent(student, title = "Stampa Scheda") {
    ...
    const studentHtml = `
        ...
        <table border="1" cellpadding="4" style="width:100%; margin-bottom:15px;">
            ...
            <td><th>Note</th><td>${escapeHtml(student.psicoAttitudinali?.note || '')}</td></tr>
        </table>
