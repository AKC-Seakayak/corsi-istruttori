...
// ===================== STAMPA CON IFRAME FUORI SCHERMO =====================
async function printCourse(title = "Stampa corso") {
    ...
    for (let i = 0; i < courseStudents.length; i++) {
        ...
        studentsHtml += `<div style="${pageBreak}">${logoHtml}
            ...
            <h4>Attrezzatura</h4><tr><th>Valutazione</th><td>${getValutazioneLabel(s.attrezzatura?.score)}</td></tr>
            <tr><th>Note</th><td>${escapeHtml(s.attrezzatura?.note||'')}</td></tr></table>
            <h4>Valutazioni didattiche</h4><table border="1"><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>
            ${didatticheItems.map(item => {
                const v = s.didattiche?.[item] || {};
                return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note||'')}</td></tr>`;
            }).join('')}</tbody></table>
            <h4>Capacità psico-attitudinali</h4><tr><th>Valutazione</th><td>${getValutazioneLabel(s.psicoAttitudinali?.score)}</td></tr>
            <tr><th>Note</th><td>${escapeHtml(s.psicoAttitudinali?.note||'')}</td></td></tr>
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `${logoHtml}
        ...
        <h4>Attrezzatura</h4></table><th>Valutazione</th><td>${getValutazioneLabel(student.attrezzatura?.score)}</td></tr>
        ...
        <h4>Capacità psico-attitudinali</h4><table><th>Valutazione</th><td>${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr>
        <tr><th>Note</th><td>${escapeHtml(student.psicoAttitudinali?.note||'')}</td></tr><td>
...
async function exportStudentAsPDF() {
    ...
    const didatticheItems = getDidatticheItemsForCourse(course.level);
    const logoHtml = `<div style="text-align:center; margin-bottom:15px;"><img src="logo-fict.png" style="max-width:180px;"></div>`;
    const itemsTech = evaluationItems.map(item => {
        const v = student.evaluations?.[item] || {};
        return `<p><strong>${escapeHtml(item)}</strong>: ${getValutazioneLabel(v.score)}<br><em>Note:</em> ${escapeHtml(v.note||'')}</p>`;
    }).join('');
    const itemsDidat = didatticheItems.map(item => {
        const v = student.didattiche?.[item] || {};
        return `<p><strong>${escapeHtml(item)}</strong>: ${getValutazioneLabel(v.score)}<br><em>Note:</em> ${escapeHtml(v.note||'')}</p>`;
    }).join('');
    const studentHtml = `${logoHtml}
    ...
    try {
        ...
        await html2pdf().set(opt).from(element).save();
        showMessage('PDF esportato con successo!', 'success');
        // Ripristino vista su mobile dopo esportazione
        setTimeout(() => {
