...
async function printCourse(title = "Stampa corso") {
    ...
    const evaluationItems = getEvaluationItemsForCourse(course.level);
    const didatticheItems = getDidatticheItemsForCourse(course.level);
    const logoHtml = `<div style="text-align:center;"><img src="logo-fict.png" style="max-width:180px;"></div>`;
    let studentsHtml = '';
    for (let i = 0; i < courseStudents.length; i++) {
        const s = courseStudents[i];
        const pageBreak = i === 0 ? '' : 'page-break-before: always;';
        studentsHtml += `<div style="${pageBreak}">${logoHtml}
            ...
            <h4>Valutazioni tecniche</h4><table border="1" cellpadding="4" style="border-collapse:collapse;width:100%"><thead><tr><th>Abilità</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>
            ...
            <h4>Attrezzatura</h4><table><tr><th>Valutazione</th><td>${getValutazioneLabel(s.attrezzatura?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(s.attrezzatura?.note||'')}</td></tr></table>
            <h4>Valutazioni didattiche</h4><table><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>
            ${didatticheItems.map(item => {
                const v = s.didattiche?.[item] || {};
                return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note||'')}</td></tr>`;
            }).join('')}</tbody></table>
            <h4>Capacità psico-attitudinali</h4><table><tr><th>Valutazione</th><td>${getValutazioneLabel(s.psicoAttitudinali?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(s.psicoAttitudinali?.note||'')}</td></tr></table>
            <h4>Giudizio finale e TESI esame</h4><p>${escapeHtml(s.giudizioFinale||'')}</p></div>`;
    }
    ...
    if (!win) { alert("Permetti i popup per questo sito."); return; }
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `${logoHtml}
        ...
        <h4>Valutazioni tecniche</h4><table border="1"><thead><tr><th>Abilità</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>
        ${evaluationItems.map(item => {
            const v = student.evaluations?.[item] || {};
            return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note||'')}</td></tr>`;
        }).join('')}</tbody></table>
        <h4>Attrezzatura</h4><table><tr><th>Valutazione</th><td>${getValutazioneLabel(student.attrezzatura?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(student.attrezzatura?.note||'')}</td></tr></table>
        <h4>Valutazioni didattiche</h4><table><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>
        ${didatticheItems.map(item => {
            const v = student.didattiche?.[item] || {};
            return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note||'')}</td></tr>`;
        }).join('')}</tbody></table>
        <h4>Capacità psico-attitudinali</h4><table><tr><th>Valutazione</th><td>${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr><tr><th>Note</th><td>${escapeHtml(student.psicoAttitudinali?.note||'')}</td></tr></table>
...
async function exportCourseAsPDF() {
    ...
    for (let i = 0; i < courseStudents.length; i++) {
        ...
        studentsHtml += `<div style="${pageBreak}">${logoHtml}
            ...
            ${evaluationItems.map(item => {
                const v = s.evaluations?.[item] || {};
                return `<p><strong>${escapeHtml(item)}</strong>: ${getValutazioneLabel(v.score)}<br><em>Note:</em> ${escapeHtml(v.note||'')}</p>`;
            }).join('')}
    ...
    const fullHtml = `<!DOCTYPE html><html><head><title>${escapeHtml(course.name)}</title><style>body{font-family:Inter,sans-serif;margin:20px} img{max-width:180px} h3{page-break-before:always}</style></head><body><h1>${escapeHtml(course.name)}</h1><p>Esportato il ${new Date().toLocaleString()}</p>${studentsHtml}</body></html>`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    registerBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            await window.setDoc(window.doc(window.db, "users", u.user.uid), {
                email: email,
                role: "user",
                uid: u.user.uid
            });
    ...
    async function showStudentEditView(student) {
        ...
        for (let item of evaluationItems) {
            ...
            techRows += `<tr><td><strong>${safeItem}</strong></td>
        ...
        for (let item of didatticheItems) {
            ...
            const safeItem = escapeHtml(item);
            didatRows += `<tr><td><strong>${safeItem}</strong></td>
        ...
        const formHTML = `<div class="form-section">
            ...
            <div class="accordion-section" id="accordion-tech"><div class="accordion-header">Valutazioni tecniche</div><div class="accordion-content"><div style="overflow-x:auto;"><table class="evaluation-table"><tbody>${techRows}</tbody></table></div></div></div>
            <div class="accordion-section" id="accordion-attrezzatura"><div class="accordion-header">Attrezzatura</div><div class="accordion-content"><div class="single-evaluation"><select id="attrezzatura-select" class="evaluation-select" style="width:100%">${makeSelect(attrezzatura.score)}</select><div class="note-cell"><textarea id="attrezzatura-note" rows="2" style="flex:1">${escapeHtml(attrezzatura.note)}</textarea><button id="delete-attrezzatura-note" class="delete-note-btn">🗑 Cancella</button></div></div></div></div>
            <div class="accordion-section" id="accordion-didat"><div class="accordion-header">Valutazioni didattiche</div><div class="accordion-content"><div style="overflow-x:auto;"><table class="evaluation-table"><tbody>${didatRows}</tbody></table></div></div></div>
            <div class="accordion-section" id="accordion-psico"><div class="accordion-header">Capacità psico-attitudinali</div><div class="accordion-content"><div class="single-evaluation"><select id="psico-select" class="evaluation-select" style="width:100%">${makeSelect(psicoAttitudinali.score)}</select><div class="note-cell"><textarea id="psico-note" rows="2" style="flex:1">${escapeHtml(psicoAttitudinali.note)}</textarea><button id="delete-psico-note" class="delete-note-btn">🗑 Cancella</button></div></div></div></div>
            <div class="accordion-section" id="accordion-giudizio"><div class="accordion-header">Giudizio finale e TESI esame</div><div class="accordion-content"><div class="giudizio-finale"><textarea id="giudizio-finale" rows="3" style="width:100%">${escapeHtml(giudizioFinale)}</textarea></div></div></div>
