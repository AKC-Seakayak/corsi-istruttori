...
const evaluationItemsLevel1 = [
    ...
    "Pagaiata circolare 360°", "Spostamento a un tempo", "Spostamento laterale continuo",
...
async function printCourse(title = "Stampa corso") {
    ...
    for (let i = 0; i < courseStudents.length; i++) {
        ...
        for (let item of evaluationItems) {
            ...
            techRows += `<tr><td style="border:1px solid #ccc; padding:4px;"><strong>${escapeHtml(item)}</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(v.score)}</td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(v.note || '')}</td></tr>`;
        }
        let didatRows = '';
        for (let item of didatticheItems) {
            const v = s.didattiche?.[item] || {};
            didatRows += `<tr><td style="border:1px solid #ccc; padding:4px;"><strong>${escapeHtml(item)}</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(v.score)}</td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(v.note || '')}</td></tr>`;
        }
        studentsHtml += `<div style="${pageBreak}">
            ${logoHtml}
            <h3>${escapeHtml(s.name)} ${escapeHtml(s.surname)}</h3>
            <p><strong>Nato:</strong> ${s.birthDate || '-'}<br>
            <strong>Indirizzo:</strong> ${escapeHtml(s.address || '-')}<br>
            <strong>Telefono:</strong> ${escapeHtml(s.phone || '-')}<br>
            ...
            <table style="width:100%; border-collapse:collapse;">${techRows}</td>
            <h4>Attrezzatura</h4>
            <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(s.attrezzatura?.score)}</td></tr>
            <tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(s.attrezzatura?.note || '')}</td></tr></table>
            <h4>Valutazioni didattiche</h4>
            <table style="width:100%; border-collapse:collapse;"><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${didatRows}</tbody></table>
            <h4>Capacità psico-attitudinali</h4>
            <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(s.psicoAttitudinali?.score)}</td></tr>
            <tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(s.psicoAttitudinali?.note || '')}</td></tr></table>
            <h4>Giudizio finale e TESI esame</h4>
            <p>${escapeHtml(s.giudizioFinale || '')}</p>
        </div>`;
    }
    const fullHtml = `<!DOCTYPE html><html><head><title>${escapeHtml(course.name)}</title><style>body{font-family:Inter,sans-serif;margin:20px} img{max-width:180px}</style></head><body><h2>${escapeHtml(course.name)}</h2><p>Stampato il ${new Date().toLocaleString()}</p>${studentsHtml}</body></html>`;
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `${logoHtml}
        ...
        <h4>Attrezzatura</h4>
        <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(student.attrezzatura?.score)}</td></tr>
        <tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(student.attrezzatura?.note || '')}</td></tr></table>
        <h4>Valutazioni didattiche</h4>
        <table style="width:100%; border-collapse:collapse;"><thead><tr><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead><tbody>${didatRows}</tbody></table>
        <h4>Capacità psico-attitudinali</h4>
        <table style="width:100%; border-collapse:collapse;"><tr><td style="border:1px solid #ccc; padding:4px;"><strong>Valutazione</strong></td><td style="border:1px solid #ccc; padding:4px;">${getValutazioneLabel(student.psicoAttitudinali?.score)}</td></tr>
        <tr><td style="border:1px solid #ccc; padding:4px;"><strong>Note</strong></td><td style="border:1px solid #ccc; padding:4px;">${escapeHtml(student.psicoAttitudinali?.note || '')}</td></tr></table>
        <h4>Giudizio finale e TESI esame</h4>
        <p>${escapeHtml(student.giudizioFinale || '')}</p>`;
    const fullHtml = `<!DOCTYPE html><html><head><title>${escapeHtml(student.name)} ${escapeHtml(student.surname)}</title><style>body{font-family:Inter,sans-serif;margin:20px} img{max-width:180px}</style></head><body><h2>${escapeHtml(course.name)}</h2><p>Stampato il ${new Date().toLocaleString()}</p>${studentHtml}</body></html>`;
    const iframe = document.createElement('iframe');
    iframe.style.position = 'absolute';
    iframe.style.left = '-9999px';
    iframe.style.top = '-9999px';
...
function showAutoSaveMessage(msg, isError = false) {
...
async function performAutoSave() {
    if (isSaving) return;
    isSaving = true;
    showAutoSaveMessage('Salvataggio...');
    ...
    try {
        ...
        const medicalCert = document.getElementById('medical-cert')?.checked || false;
        const blsdCert = document.getElementById('blsd-cert')?.checked || false;
        const otherPatents = document.getElementById('other-patents')?.value || '';
        
        const course = coursesList.find(c => c.id === currentCourseId);
        const evaluationItemsList = course ? getEvaluationItemsForCourse(course.level) : [];
        const evaluationsData = {};
        for (let item of evaluationItemsList) {
        ...
        const didatticheItemsList = course ? getDidatticheItemsForCourse(course.level) : [];
        const didatticheData = {};
        for (let item of didatticheItemsList) {
        ...
        const attData = {
            score: document.getElementById('attrezzatura-select')?.value || "0",
            note: document.getElementById('attrezzatura-note')?.value || ''
        };
        const psicoData = {
            score: document.getElementById('psico-select')?.value || "0",
        ...
        const studentData = {
            ...
            attrezzatura: attData,
            psicoAttitudinali: psicoData,
            giudizioFinale: giudizio,
            courseId: currentCourseId,
            updatedAt: new Date().toISOString()
        };
        
        if (currentEditingStudent && currentEditingStudent.id) {
            await window.updateDoc(window.doc(window.db, "students", currentEditingStudent.id), studentData);
            showAutoSaveMessage('Salvato');
        } else {
            const docRef = await window.addDoc(window.collection(window.db, "students"), studentData);
            currentEditingStudent = { id: docRef.id, ...studentData };
            showAutoSaveMessage('Creato e salvato');
        }
    } catch (err) {
        console.error(err);
        showAutoSaveMessage('Errore salvataggio', true);
    } finally {
        isSaving = false;
    }
}
...
function attachAutoSaveEvents() {
    ...
    // Anche per i pulsanti di valutazione (se ci fossero bottoni, ma attualmente sono select)
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('export-course-pdf-btn')?.addEventListener('click', () => printCourse("Esporta PDF corso (stampa e salva come PDF)"));
    document.getElementById('export-student-pdf-btn')?.addEventListener('click', () => {
        if (currentEditingStudent) printSingleStudent(currentEditingStudent, "Esporta PDF scheda (stampa e salva come PDF)");
        else alert('Nessun allievo selezionato');
    });
    ...
    async function showStudentEditView(student) {
        ...
        // ATTIVA AUTO-SALVATAGGIO
        attachAutoSaveEvents();
        // Inoltre, attacca manualmente ai select delle valutazioni (che sono già nel form)
        const selects = container.querySelectorAll('.evaluation-select');
        selects.forEach(sel => {
