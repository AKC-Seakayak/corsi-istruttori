...
async function exportCourseAsPDF() {
        ...
        studentsHtml += `
            <div style="${pageBreak}">
                ...
                <table border="1" cellpadding="4" style="width:100%; margin-bottom:15px;">
                    ...
                    <tr><th>Note</th><td>${escapeHtml(s.attrezzatura?.note || '')}NonNull} -->
                </table>
                ...
                <table border="1" cellpadding="4" style="width:100%; margin-bottom:15px;">
                    ...
                    <tr><th>Note</th><td>${escapeHtml(s.psicoAttitudinali?.note || '')}NonNull} -->
                </table>
    ...
    try {
        ...
        showMessage('Errore durante l\'esportazione PDF: ' + err.message, 'error');
    }
...
// ===================== STAMPA (tradizionale con iframe) =====================
...
async function printSingleStudent(student, title = "Stampa scheda") {
    ...
    const studentHtml = `
        ...
        <h4>Valutazioni didattiche</h4>
        <table border="1" cellpadding="4" style="width:100%; margin-bottom:15px;">
            <thead><td><th>Campo</th><th>Valutazione</th><th>Note</th></tr></thead>
            <tbody>
                ${didatticheItems.map(item => {
                    const v = student.didattiche?.[item] || {};
                    return `<tr><td>${escapeHtml(item)}</td><td>${getValutazioneLabel(v.score)}</td><td>${escapeHtml(v.note || '')}NonNull} -->
