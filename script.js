...
// LISTA MANOVRE CORRETTA (maiuscole iniziali, voci separate)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    printCourseBtn.addEventListener('click', async () => {
        ...
        printWindow.document.write(`
            ...
            ${courseStudents.map(s => `
                ...
                <h3>Attrezzatura</h3>
                <table><th>Valutazione</th><td>${getValutazioneLabel(s.attrezzatura?.score)}</td></tr>
                <tr><th>Note</th><td>${escapeHtml(s.attrezzatura?.note || '')}</td></tr>
                </table>
                <h3>Valutazioni didattiche</h3>
                <table><th>Campo</th><th>Valutazione</th><th>Note</th></tr>
    ...
    async function showStudentEditView(student) {
        ...
        function makeSelect(currentValue) {
            return valutazioneOptions.map(opt => `<option value="${opt.value}" ${currentValue == opt.value ? 'selected' : ''}>${opt.label}</option>`).join('');
        }

        // Funzioni per creare le sezioni accordion (contenuto già generato)
        const techRows = evaluationItems.map(item => {
        ...
        const didatRows = didatticheItems.map(item => {
        ...
        const formHTML = `
            <div class="form-section">
                ...
                <!-- Sezione Valutazioni tecniche (accordion) -->
                <div class="accordion-section" id="accordion-tech">
                    <div class="accordion-header">📋 Valutazioni tecniche</div>
                    <div class="accordion-content">
                        <div style="overflow-x:auto;"><table class="evaluation-table"><tbody>${techRows}</tbody></table></div>
                    </div>
                </div>
                
                <!-- Sezione Attrezzatura (accordion) -->
                <div class="accordion-section" id="accordion-attrezzatura">
                    <div class="accordion-header">⚙️ Attrezzatura</div>
                    <div class="accordion-content">
                ...
                <!-- Sezione Valutazioni didattiche (accordion) -->
                <div class="accordion-section" id="accordion-didat">
                    <div class="accordion-header">📖 Valutazioni didattiche</div>
                    <div class="accordion-content">
                        <div style="overflow-x:auto;"><table class="evaluation-table"><tbody>${didatRows}</tbody></table></div>
                    </div>
                </div>
                
                <!-- Sezione Capacità psico-attitudinali (accordion) -->
                ...
                <!-- Sezione Giudizio finale (accordion) -->
        ...
        // Gestione cancellazione note (tecniche/didattiche)
